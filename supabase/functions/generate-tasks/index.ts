...
const SAVARANKISKAS_MIN_TASKS = 5;
...
function buildSavarankiskasDifficultyBlock(
  ...
  return `SAVARANKISKO DARBO RĖŽIMAS — mišrus sunkumas pagal mokytojo nurodytą temą ar kelias temas:
Visos užduotys turi priklausyti tai pačiai tematikai (temoms), aprašytai vartotojo prašyme.

• ${counts.lengvos} LENGVŲ užduočių: ${buildDifficultyDescription("lengvos", grade)}
• ${counts.vidutinės} VIDUTINIO SUNKUMO užduočių: ${buildDifficultyDescription("vidutinės", grade)}
• ${counts.sunkios} SUNKIŲ užduočių: ${buildDifficultyDescription("sunkios", grade)}

PRIVALOMA: grąžink tiksliai nurodytą kiekvieno lygio užduočių skaičių. Jei mokytojas nurodė kelias temas — paskirstyk jas tarp užduočių. Užduotis gali eiti bet kokia tvarka.`;
}
...
${
  difficulty === "savarankiskas"
    ...
    : `Sunkumo lygis: ${buildDifficultyDescription(difficulty, grade)}`
...
function buildUserMessage(taskCount: number, prompt: string, difficulty?: string): string {
  const topicHint =
    difficulty === "savarankiskas"
      ? "\n\nSavarankiško darbo režimas: visos užduotys turi atitikti aprašytą temą (-as), bet ~40% turi būti lengvos, ~40% vidutinio sunkumo, ~20% sunkios."
      : "";
...
function buildImageUserContent(
  ...
  const savarankiskasHint =
    difficulty === "savarankiskas"
      ? " Savarankiško darbo režimas: ~40% lengvų, ~40% vidutinio sunkumo, ~20% sunkių pagal temą."
      : "";

  const textPart = prompt && prompt.trim().length >= 3
    ? `Nuotraukoje pavaizduota užduotis. Sugeneruok ${taskCount} panašių užduočių.${savarankiskasHint}\n\nPapildomas aprašymas: ${prompt}\n\nGrąžink tik JSON objektą su "tasks" masyvu.`
    : `Nuotraukoje pavaizduota matematikos užduotis. Nuskaityk ją ir sugeneruok ${taskCount} panašių užduočių.${savarankiskasHint}\n\nGrąžink tik JSON objektą su "tasks" masyvu.`;
...
if (userProfile) {
  ...
  if (taskCount > 3) {
    ...
    if (effectiveDifficulty === "savarankiskas") {
      if (userProfile?.plan !== "pro") {
        return new Response(
          JSON.stringify({ error: "Savarankiško darbo režimas prieinamas tik PRO plano vartotojams." }),
          { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
      if (taskCount < SAVARANKISKAS_MIN_TASKS) {
        return new Response(
          JSON.stringify({
            error: `Savarankiško darbo režime reikia bent ${SAVARANKISKAS_MIN_TASKS} užduočių.`,
          }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
        );
      }
    ...
    const userContent = imageBase64
      ? buildImageUserContent(taskCount, prompt, imageBase64, effectiveDifficulty)
      : buildUserMessage(taskCount, prompt, effectiveDifficulty);
